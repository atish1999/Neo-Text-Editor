 ...
 *
 * <p>This class is used for console mode programs. It supports non-blocking reads of single key
 * strokes without echo.
 */
public class RawConsoleInput {
   ...
   *
   * @param wait <code>true</code> to wait until an input character is available, <code>false</code>
   *     to return immediately if no character is available.
   ...
   *
   * <p>On Unix this method switches the console back to echo mode. read() leaves the console in
   * non-echo mode.
   */
  ...
  private static Pointer getStdInputHandle() throws IOException {
    ...
    if (Pointer.nativeValue(handle) == 0
        || Pointer.nativeValue(handle) == Kernel32Defs.INVALID_HANDLE_VALUE) {
  ...
  // (This method is synchronized because the charsetDecoder must only be used by a single thread at
  // once.)
  ...
  private static synchronized void initUnix() throws IOException {
    ...
    if (stdinIsConsole) {
      ...
      System.out.println("not a terminal device");
    }
    initDone = true;
  }
